...
function App() {
  ...
  const [pendingConfigSwitchId, setPendingConfigSwitchId] = useState<
    string | null
  >(null);
  ...
  const effectiveThemeId =
    config.theme || (themes.length > 0 ? themes[0].id : '');
  useEffect(() => {
    fetch('/themes')
      .then(res => res.json())
  ...
  const exportCurrentConfig = useCallback(() => {
    const fileNameBase = sanitizeFileName(
      config.config_name || activeConfigId || 'config',
    );
  ...
  const switchToPresetById = useCallback(
    (presetId: string) => {
  ...
  useEffect(() => {
    ...
    const activeTheme =
      themes.find(t => t.id === effectiveThemeId) || themes[0];
    if (activeTheme) {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case 'set-up':
        return <SetUpSection {...props} />;
      case 'general':
        return (
          <>
            <EventSection {...props} />
            <RaceScheduleSection {...props} />
            <SkillSection {...props} />
          </>
        );
      case 'training':
      ...
      case 'skills':
        return <SkillListSection {...props} />;
      case 'schedule':
        return <RaceListSection {...props} />;
      case 'events':
    ...
    <main className='flex min-h-screen w-full bg-triangles overflow-hidden'>
      ...
      <div className='flex-1 flex flex-col overflow-y-auto'>
        <header className='p-6 w-full py-4 self-start border-b border-border flex items-end justify-between sticky top-0 z-10 backdrop-blur-md'>
          ...
          {toast.show && (
            <div
              className={`absolute top-14 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-1 rounded-full text-sm font-medium animate-in fade-in zoom-in duration-300 border ${
                toast.isError
          ...
          <div className='flex items-end justify-between w-full'>
            <div className='flex items-center gap-4'>
              <div className='space-y-1 relative' ref={presetActionsRef}>
                <label className='text-xs font-thin text-muted-foreground ml-1 mr-2'>
                  Configuration File
                </label>
                <Tooltips size='xs'>
                  {
                    'Configs are saved as files in the bot folder under config/.\n\
                      ...
                      <div className='absolute translate-y-1 w-64 rounded-lg border border-border bg-popover text-foreground shadow-2xl p-2 z-50'>
                        <div className='px-2 pt-1 pb-2'>
                          ...
                          <p className='text-xs text-muted-foreground'>
                            Create, duplicate, delete, import, or export
                            presets.
                          </p>
                        </div>
                        <Button
              ...
              <div
                className={`flex w-fit gap-4 transition-all duration-300 ease-out overflow-x-hidden pb-2 -mb-2 items-end ${
                  isEditing
                    ? 'max-w-200 opacity-100 translate-x-0'
              ...
              >
                ...
                <div className='space-y-1'>
                  <label className='text-xs font-thin text-muted-foreground ml-1'>
                    Name
                  </label>
                  <Input
                ...
                <div className='space-y-1'>
                  <label className='text-xs font-thin text-muted-foreground ml-1'>
                    Uma{' '}
                    <span className='text-[10px] text-slate-800/40'>
                      (Theme)
                    </span>
                  </label>
                  <Select
